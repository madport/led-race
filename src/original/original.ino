...
void draw_car1(void){
  for(int i=0;i<=global_car;i++){
    track.setPixelColor(((word)dist1 % NPIXELS)+i, track.Color(0,255,0));
    }                   
  }
...
void loop() {
   ...
   // if ((gravity_map[(word)dist1 % NPIXELS])>127) speed1+=kg*((gravity_map[(word)dist1 % NPIXELS])-127);
    ...
    /* distn = 0 added */
    if (dist1>NPIXELS) {loop1++;/*tone(PIN_AUDIO,600)*/;dist1 = 0;/*TBEEP=2;*/};
    if (dist2>NPIXELS) {loop2++;/*tone(PIN_AUDIO,700)*/;dist2 = 0;/*TBEEP=2;*/};
    Serial.print(loop1);
    /* First player wins */
